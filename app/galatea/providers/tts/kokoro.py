...
def create(cfg: dict) -> "KokoroTTS":
    """Factory for the provider registry: build KokoroTTS from a config `tts` section."""
    from galatea.config import voice_options

    vo = voice_options(cfg)
    return KokoroTTS(
        base_url=cfg.get("url") or vo.get("base_url", "http://localhost:8880/v1"),
        api_key=vo.get("api_key", "not-needed"),
        model=cfg.get("model") or vo.get("model", "kokoro"),
        voice=vo.get("voice") or vo.get("voice_id", "af_bella"),
