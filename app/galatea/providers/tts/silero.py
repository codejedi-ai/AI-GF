...
def create(cfg: dict) -> "SileroTTS":
    """Factory for the provider registry: build SileroTTS from a config `tts` section."""
    from galatea.config import voice_options

    vo = voice_options(cfg)
    return SileroTTS(
        language=vo.get("language", "en"),
        speaker=vo.get("speaker") or vo.get("voice_id", "lj_16khz"),
    )
