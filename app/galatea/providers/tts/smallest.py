"""
Smallest AI Waves TTS plugin for LiveKit agents.

Uses the Smallest AI Waves API (cloud) for text-to-speech. Waves supports
models such as lightning and lightning-large. This is the TTS side of the
LiveKit Smallest agent (Waves + Pulse).
...
def _synthesize_sync(
    ...
    client = WavesClient(
        ...
        voice_id=voice_id,
        speed=speed,
        # The SDK supports output formats: "pcm", "mp3", "wav", "mulaw".
        # We want raw PCM int16 bytes (no container) for LiveKit audio frames.
        output_format="pcm",
    )
    # synthesize returns raw audio bytes (PCM int16) when output_format="pcm"
...
def create(cfg: dict) -> "SmallestTTS":
    """Factory for the provider registry: build SmallestTTS from a config `tts` section."""
    import os as _os
    ...
    return SmallestTTS(
        api_key=vo.get("api_key") or _os.getenv("SMALLEST_API_KEY"),
        model=cfg.get("model") or vo.get("model", "lightning"),
        voice_id=vo.get("voice_id", "emily"),
        speed=vo.get("speed", 1.0),
        sample_rate=vo.get("sample_rate", SMALLEST_TTS_SAMPLE_RATE),
    )
