...
def create(cfg: dict) -> "HFTTS":
    """Factory for the provider registry: build HFTTS from a config `tts` section."""
    from galatea.config import voice_options

    vo = voice_options(cfg)
    return HFTTS(
        model=cfg.get("model") or vo.get("model", "microsoft/speecht5_tts"),
