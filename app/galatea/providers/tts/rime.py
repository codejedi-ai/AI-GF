...
def create(cfg: dict) -> "RimeTTS":
    """Factory for the provider registry: build RimeTTS from a config `tts` section."""
    from galatea.config import voice_options

    vo = voice_options(cfg)
    api_key = vo.pop("api_key", None)
    model = cfg.get("model") or vo.pop("model", "arcana")
    return RimeTTS(api_key=api_key, model=model, **vo)
