...
def create(cfg: dict) -> "ElevenLabsTTS":
    """Factory for the provider registry: build ElevenLabsTTS from a config `tts` section."""
    from galatea.config import voice_options

    vo = voice_options(cfg)
    api_key = vo.pop("api_key", None)
    model = cfg.get("model") or vo.pop("model", "eleven_multilingual_v2")
    voice_id = vo.pop("voice_id", None)
    return ElevenLabsTTS(api_key=api_key, model=model, voice_id=voice_id, **vo)
