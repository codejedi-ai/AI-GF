...
from tools import get_snowflake_rag_response, write_chat_to_snowflake

load_dotenv()
logger = logging.getLogger("voice-agent")

# Default config when --config is not passed.
DEFAULT_CONFIG_PATH = Path.home() / ".galatea" / "agent_template" / "Natasha.json"
...
def _tts_tag_block_for_cfg(cfg: dict) -> str:
    """Return the TTS expressive-tag block for Rime."""
...
def build_intro_generation_prompt(cfg: dict) -> str:
    """Build the full prompt for greeting/intro generation: intro_generation_prompt + Rime tag schema.
    Pass this to the AI that generates the intro phrase so it uses the correct tag syntax.
    """
...
class GalateaAssistant(Agent):
    def __init__(self, prompt: str) -> None:
        super().__init__(instructions=prompt)


class GalateaAssistantWithSnowflakeRAG(Agent):
...
async def entrypoint(ctx: JobContext):
    ...
    # Wait for a participant or use existing one (useful for console mode)
    participant = (
        await ctx.wait_for_participant()
        if not ctx.room.remote_participants
        else list(ctx.room.remote_participants.values())[0]
    )
    ...
    rime_opts = {
        k: v
        for k, v in vo.items()
        if k not in ("provider", "model", "url", "model_id", "voice_id", "speaker")
    }
    model = tts_model or vo.get("model_id") or "arcana"
    speaker = vo.get("speaker") or vo.get("voice_id") or "celeste"
    voice_tts = rime.TTS(model=model, speaker=speaker, **rime_opts)

    llm_prompt = build_agent_instructions(cfg)
    greeting = cfg.get("greeting") or {}
    intro_phrase = greeting.get("intro_phrase", cfg.get("intro_phrase", "Hello!"))
    # Optionally generate intro with Rime tag schema in the prompt.
    ...
    # STT: whisper (local server) or openai (cloud).
    ...
    def _on_conversation_item_added(ev):
        try:
            ...
            role = getattr(item, "role", None) or getattr(item, "message", {}).get(
                "role", "user"
            )
            text = (
            ...
            if role and str(text).strip():
                asyncio.create_task(
                    write_chat_to_snowflake(
                        session_id, participant_id, role, str(text), agent_name
                    )
                )
        except Exception as e:
    ...
    if "snowflake_rag" in tools_list:
        agent = GalateaAssistantWithSnowflakeRAG(prompt=llm_prompt)
        logger.info("Agent has Snowflake Agentic RAG tool enabled")
    else:
        agent = GalateaAssistant(prompt=llm_prompt)
...
def _collect_hf_models_from_configs(agent_template_dir: Path) -> set[str]:
    ...
    for path in agent_template_dir.glob("*.json"):
        try:
            ...
            continue
        # Only collect LLM models; TTS provider is independent of this downloader.
        llm_cfg = cfg.get("llm")
        if (
            isinstance(llm_cfg, dict)
            and (llm_cfg.get("provider") or "").lower() == "huggingface"
        ):
...
def _run_download_files() -> None:
    ...
    agent_template_dir = Path.home() / ".galatea" / "agent_template"
    model_ids = _collect_hf_models_from_configs(agent_template_dir)
    # Ensure default LLM model is always included
